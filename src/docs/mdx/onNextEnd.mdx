...
# onNextEnd
#### `onNextEnd` will pass the new __current__ `item-object` and __current__ `page-index`:  
```
...
<Playground >
    <Carousel 
        itemsToShow={2}
        onNextEnd={(currentItem, pageIndex) => alert(JSON.stringify({currentItem, pageIndex}))}
    >
